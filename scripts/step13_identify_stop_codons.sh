...
# Description: Scans the stitched multi-species alignments to locate biological stop codons (TAA, TAG, TGA) in-frame, mapping them to the reference.
# Usage: ./step13_identify_stop_codons.sh <step9_trimmed_dir> <output_dir> <bed_file> <reference_species> <step11_summary_tsv>
# ==============================================================================
...
out_master = os.path.join(output_dir, "master_stop.tsv")
...
header = "file_name\tgff_transcriptID\tChromosome\tStart\tEnd\tSpecies\tStopCodonStart_RefPos\tStopCodonEnd_RefPos\tStopCodon\tgff_strand_ref\tcds_length\treference_length\n"
all_rows = []
...
for aln_file in glob.glob(os.path.join(input_dir, "*.aln")):
    ...
    # Pull attributes from summary map based on corrected columns
    ...
    ref_id = next((sp for sp in order if reference_species in sp), None)
    ...
    # Generate individual file output as requested
    with open(os.path.join(output_dir, f"{file_name}_stops.tsv"), "w") as ind:
        ind.write(header)
        for sp in order:
...
print(f"Stop codons done. Master summary at {out_master}")
EOF